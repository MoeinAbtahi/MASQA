...
#### **Steps**:
1. **Upload CSV File**: Users upload the extracted CSV file.
2. **Select Issues**: From the uploaded CSV, users can select individual files with issues.
3. **View & Edit**:
   - View the selected file's content and its associated issues.
   - A **preset prompt** is generated based on the file content and issue details.
   - Users can **edit the prompt** as needed before sending it to the OpenAI API.
4. **Choose GPT Model**: Select the desired GPT model (e.g., GPT-3.5 Turbo or GPT-4o).
5. **Send Prompt**: The prompt is sent to the GPT API.
6. **View & Save Revised Code**: The revised version of the file, generated by the AI, is displayed for review. Users can save the revised file.
